...
| Shared link shows no picture | The picture needs a full `https://…` address, which the build only knows on GitHub Pages (it reads it from Pages itself) or when you set a `SITE_URL` environment variable — on Vercel, add `SITE_URL` with your site's address in project settings. Also check `public/og.png` still exists in your repo — that file *is* the card, and no config line is needed to use it. WhatsApp and Facebook cache previews for days, so test with a fresh link (add `?1` to the end) rather than re-sending the old one. |
| "Pay directly" does nothing on my phone | Known GPay/PhonePe limitation for browser payments to personal UPI IDs — not a bug in your page. It's marked experimental for exactly this reason, and the QR and Copy-UPI-ID paths sit right above it. |
